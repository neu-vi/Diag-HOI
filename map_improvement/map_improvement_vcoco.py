...
class VCOCOeval(object):
    ...
    def precompute_data(self, gt_all_boxes, gt_human_inds, gt_human_boxes, gt_actions, gt_roles_all, ignore, pred_agents, pred_roles, ovr_thresh=0.5):
        ...
        all_pred_agent_boxes = np.concatenate(pred_agent_boxes_list, axis=0) # (nx25) x 4
        all_pred_role_boxes = np.concatenate(pred_role_boxes_list, axis=0) # (nx25) x 4

        top_100_preds = np.array(pred_scores_list).argsort()[::-1][:100]
        best_matching_map = -np.ones(len(gt_hois))
        for i, gt_hoi in enumerate(gt_hois):
            for idx in top_100_preds:
                if matching_map[i] == idx:
                    break
                if idx in best_matching_map:
                    continue
                if idx in matching_map:
                    continue

                agent_box = gt_human_boxes[np.where(gt_human_inds==gt_hoi[0])[0][0]]
                if gt_hoi[1] != -1:
                    role_box = gt_all_boxes[gt_hoi[1]]
                aid = gt_hoi[2]
                rid = gt_hoi[3]
                ov_agent = get_overlap(np.array([all_pred_agent_boxes[idx].tolist()]), agent_box)[0] # only got one box to compare
                if gt_hoi[1] != -1:
                    ov_role = get_overlap(np.array([all_pred_role_boxes[idx].tolist()]), role_box)[0]
                else:
                    ov_role = 0.0
                if ov_agent >= 0.5:
                    if len(np.where(top_100_preds==matching_map[i])[0])==0:
                        best_matching_map[i] = idx
                        break
                    if np.where(top_100_preds==matching_map[i])[0][0] > np.where(top_100_preds==idx)[0][0] or matching_map[i] == -1:
                        best_matching_map[i] = idx
                        break
                elif ov_role >= 0.5:
                    if len(np.where(top_100_preds==matching_map[i])[0])==0:
        ...
        return matching_map, error_type_pred, best_matching_map, pred_scores_list, gt_hois, flag_list, flag_idx_list, sc_pred

    # eval type: scenario_1
    def _do_role_eval(self, vcocodb, detections_file, fix_type, logger, model_param_path, model_name):
        ...
        for i in tqdm.tqdm(range(len(vcocodb))):
            ...
            if is_exist == 0:
                ...
                matching_map, error_type_pred, best_matching_map, pred_scores_list, gt_hois, flag_list, flag_idx_list, sc_pred = \
                ...
                inter_param['best_matching_map'].append(best_matching_map)
                ...
                best_matching_map = inter_param['best_matching_map'][i]
            ...
            elif fix_type == "dup" or fix_type == "both":
                self.fix_dup_or_both(fp, tp, sc, error_type_pred, pred_scores_list, fix_type)
            elif fix_type == "hum":
                self.fix_human(fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred)
            elif fix_type == "obj":
                self.fix_object(fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred)
            elif fix_type == "action":
                self.fix_action(fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred)
            elif fix_type == "assoc":
                self.fix_assoc(fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred)
            elif fix_type == "missing":
                self.fix_missing(fp, tp, sc, error_type_pred, pred_scores_list, matching_map, best_matching_map, gt_hois, npos)
            elif fix_type == "fp":
                self.fix_fp(fp, tp, sc, error_type_pred, pred_scores_list)
            elif fix_type == "fn":
                self.fix_fn(fp, tp, sc, error_type_pred, pred_scores_list, matching_map, gt_hois, npos)
    ...
    def fix_dup_or_both(self, fp, tp,sc, error_type_pred, pred_scores_list, fix_type):
        cnt = 0
        for aid in range(self.num_actions):
            if len(self.roles[aid])<2:
                # if action has no role, then no role AP computed
            ...
            for rid in range(len(self.roles[aid])-1):
                for flag in error_type_pred[aid][rid]:
                    if flag == -1:
                        ...
                        if flag==6 and fix_type == 'dup':
                            cnt += 1
                            continue
                        if flag==3 and fix_type == 'both':
                            cnt += 1
                            continue
                        fp[aid][rid].append(1)
                        tp[aid][rid].append(0)
    ...
    def fix_human(self, fp, tp,sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred):

        for i in range(len(matching_map)):
            pred_i = int(best_matching_map[i])
            gt_hoi = gt_hois[i]
            aid_gt, rid_gt = gt_hoi[2], gt_hoi[3]
            if pred_i==-1:
                continue
            ori_pred_i = int(matching_map[i])

            if flag_list[pred_i] == 1:
                (aid_pred, rid_pred, flag_idx_pred) = flag_idx_list[pred_i]
                sc_pred[aid_gt][rid_gt].append(sc_pred[aid_pred][rid_pred][flag_idx_pred])
                error_type_pred[aid_gt][rid_gt].append(0)
                error_type_pred[aid_pred][rid_pred][flag_idx_pred] = -1

                if ori_pred_i != -1:
                    (ori_aid, ori_rid, ori_flag_idx) = flag_idx_list[ori_pred_i]
                    error_type_pred[ori_aid][ori_rid][ori_flag_idx] = -1
            ...
            for rid in range(len(self.roles[aid])-1):
                ...
                for flag in error_type_pred[aid][rid]:
                    if flag == 0:
                        ...
                        fp[aid][rid].append(1)
                        tp[aid][rid].append(0)
                    sc[aid][rid].append(sc_pred[aid][rid][cnt])
                    cnt += 1
        return fp, tp, sc

    def fix_object(self, fp, tp,sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred):
    ...
    def fix_action(self, fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred):
    ...
    def fix_assoc(self, fp, tp, sc, error_type_pred, matching_map, best_matching_map, gt_hois, flag_list, flag_idx_list, sc_pred):
    ...
    def fix_missing(self, fp, tp, sc, error_type_pred, pred_scores_list, matching_map, best_matching_map, gt_hois, npos):

        for i in range(len(matching_map)):
            pred_i = int(best_matching_map[i])
            gt_hoi = gt_hois[i]
            aid, rid = gt_hoi[2], gt_hoi[3]

            ori_pred_i = int(matching_map[i])

            if pred_i==-1 and ori_pred_i==-1:
                npos[aid][rid] -= 1
        ...
        for aid in range(self.num_actions):
            ...
            for rid in range(len(self.roles[aid])-1):
                for flag in error_type_pred[aid][rid]:
                    ...
                    sc[aid][rid].append(pred_scores_list[cnt])
                    cnt += 1
        return fp, tp, sc, npos

    def fix_fp(self, fp, tp, sc, error_type_pred, pred_scores_list):
    ...
    def fix_fn(self, fp, tp, sc, error_type_pred, pred_scores_list, matching_map, gt_hois, npos):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fix_type', default='none', type=str, help='fix which type of error none/dup/both/hum/obj/action/assoc/missing')

    parser.add_argument('--save_inter_param_path', default='./params_vcoco')
    ...
    vcocoeval._do_eval(args.model_det_file, args.fix_type, logger, args.save_inter_param_path, args.model_name)
